          ...
          Consumer(
            builder: (context, ref, _) {
              ...
              return dataAsync.maybeWhen(
                data: (payload) {
                  final stats = payload.widthStats;
                  if (stats == null) return const SizedBox.shrink();
                  return _TrajectoryWidthStatsSection(stats: stats);
                },
...
/// 軌跡寬度統計區塊。
class _TrajectoryWidthStatsSection extends StatelessWidget {
  const _TrajectoryWidthStatsSection({required this.stats});

  final TrajectoryWidthStats stats;
  ...
  Widget build(BuildContext context) {
    ...
    // 格式化公尺數值
    String fmtM(double? v) => v != null ? '${(v * 100).toStringAsFixed(1)} cm' : '-';
    String fmtPct(double? v) => v != null ? '${v.toStringAsFixed(1)}%' : '-';
        ...
        Container(
          ...
          child: Column(
            children: [
              ...
              _StatRow(
                label: '最寬圈',
                value: stats.widestLapIndex != null
                    ? '第 ${stats.widestLapIndex} 圈 (${fmtM(stats.widestLapWidthM)})'
                    : '-',
              ...
              _StatRow(
                label: '最窄圈',
                value: stats.narrowestLapIndex != null
                    ? '第 ${stats.narrowestLapIndex} 圈 (${fmtM(stats.narrowestLapWidthM)})'
                    : '-',
    ...
    return Row(
      children: [
        Container(
          ...
          height: 32,
          decoration: BoxDecoration(
            color: isDark ? const Color(0xFF1A1A1A) : colors.surfaceContainerHighest,
